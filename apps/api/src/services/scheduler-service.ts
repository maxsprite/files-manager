...
export class SchedulerService {
  ...
  private tasks: Map<string, cron.ScheduledTask> = new Map<string, cron.ScheduledTask>();

  /**
   * Registers a new task in the scheduler
   */
  registerJob(name: string, schedule: string, handler: () => Promise<unknown>, active = true): void {
  ...
  startAllJobs(): void {
    ...
    console.log(`Scheduler service started with ${this.tasks.size} active jobs`);
  }
