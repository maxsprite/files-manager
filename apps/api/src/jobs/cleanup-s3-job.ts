...
export const CLEANUP_S3_SCHEDULE = process.env.CLEANUP_S3_SCHEDULE || '0 3 * * *';
 ...
 */
export async function cleanupS3OrphanedFilesJob(): Promise<{ deleted: number, total: number }> {
  ...
  try {
    ...
    console.log(`[JOB] S3 cleanup job completed. Deleted ${result.deleted} of ${result.total} files.`);
    return result;
  } catch (error) {
